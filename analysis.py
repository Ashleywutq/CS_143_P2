...
state_data = pd.read_csv("q3.csv/q3.csv")
...
shp_info = m.readshapefile('shape/st99_d00','states',drawbounds=True)  # No extension specified in path here.
pos_data = dict(zip(state_data.author_flair_text, state_data.pos))
neg_data = dict(zip(state_data.author_flair_text, state_data.neg))
diff_data = dict(zip(state_data.author_flair_text, state_data.pos - state_data.neg))
...
for nshape, seg in enumerate(m.states):
    ...
    if statenames[nshape] not in ['District of Columbia', 'Puerto Rico']:
        if statenames[nshape] == 'Alaska':
            seg = list(map(lambda xy: (0.25 * xy[0] + 800000, 0.25 * xy[1] - 900000), seg))
        if statenames[nshape] == 'Hawaii':
            seg = list(map(lambda xy: (0.5 * xy[0] + 3200000, 0.5 * xy[1] - 700000), seg))
...
for nshape, seg in enumerate(m.states):
    ...
    if statenames[nshape] not in ['District of Columbia', 'Puerto Rico']:
        ...
        poly = Polygon(seg, facecolor=color, edgecolor=color)
        ax.add_patch(poly)
plt.title('Pos - Neg Trump Sentiment Across the US')
...
story = pd.read_csv("q4_s.csv/q4_s.csv")
plt.figure(figsize=(12,5))
fig = plt.figure()
...
story = pd.read_csv("q4_c.csv/q4_c.csv")
plt.figure(figsize=(12,5))
fig = plt.figure()
ax1 = fig.add_subplot(111)

ax1.scatter(story['cscore'], story['avg(pos)'], s=10, c='b', marker="s", label='Positive')
ax1.scatter(story['cscore'], story['avg(neg)'], s=10, c='r', marker="o", label='Negative')
plt.legend(loc='lower right')
